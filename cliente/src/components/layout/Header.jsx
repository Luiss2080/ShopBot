...
export default function Header({ toggleCart, cartCount, openLogin }) {
  const location = useLocation();
  const isActive = (path) => location.pathname === path;

  return (
    <header className="sticky top-0 z-40 bg-surface/80 backdrop-blur-md border-b border-slate-200 shadow-sm transition-colors">
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <nav className="hidden lg:flex items-center gap-8 ml-10">
              ...
              <Link to="/acerca" className={`text-sm font-medium transition-colors ${isActive('/acerca') ? 'text-brand-600' : 'text-slate-600 hover:text-slate-900'}`}>
                Acerca de
              </Link>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button className="hidden sm:flex p-2 text-slate-400 hover:text-brand-500 hover:bg-slate-100 rounded-xl transition-colors">
              <Bell size={20} />
            </button>

            <div 
              onClick={openLogin}
              className="hidden sm:flex items-center gap-2 p-1.5 px-3 border border-slate-200 rounded-2xl cursor-pointer hover:shadow-sm transition-all"
            >
